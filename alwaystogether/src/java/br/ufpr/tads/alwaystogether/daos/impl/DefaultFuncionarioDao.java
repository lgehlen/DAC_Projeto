...
public class DefaultFuncionarioDao implements FuncionarioDao {
   ...
   Connection con = new ConnectionFactory().getConnection();
    ...
    public void atualizarFuncionario(Funcionario funcionario) {
        ...
        ResultSet rs = null;
        try {
            st = con.prepareStatement("UPDATE always.Funcionario SET nomeFuncionario = ?, email = ?, senha = MD5(?), regra = ?"
                                        + " WHERE idFuncionario = ?");
            st.setString(1, funcionario.getNome());
            st.setString(2, funcionario.getEmail());
            st.setString(3, funcionario.getSenha());
    ...
    public List<Funcionario> listarFuncionarios() {
        ...
        ResultSet rs = null;
        try {
            ps = con.prepareStatement("SELECT nomeFuncionario, email, idFuncionario, regra FROM always.Funcionario wHERE idFuncionario != 1 AND isRemovido != 1  ");
            rs = ps.executeQuery();
    ...
    public Funcionario buscarPorEmail(String email) {
        ...
        ResultSet rs = null;
        try {
            ps = con.prepareStatement("SELECT nomeFuncionario, regra, email, idFuncionario FROM always.funcionario WHERE email = ? AND isRemovido != 1");
    ...
    public Login buscarPorLogin(String email, String senha) {
        ...
        ResultSet rs = null;
        try {
            ps = con.prepareStatement("SELECT idFuncionario, nomeFuncionario FROM always.Funcionario WHERE email = ? AND senha = MD5(?) AND isRemovido != 1");
            ps.setString(1, email);
